...
## 1.0.0.22 - Hotfix responsivo Perfis e Permissões

- Corrigida quebra responsiva da matriz de permissões em desktop médio/notebook.
- Evitado corte das colunas Operador, Gestor/PCM, Admin e Fixo.
- Cards de categoria passam a ocupar uma coluna quando a área central não comportar a matriz em duas colunas.
- Adicionadas proteções de largura, `box-sizing` e comportamento adaptativo para switches.
- Preservados backend, API, schema, login, permissões reais e salvamento existente.

## 1.0.0.21 - Hotfix Perfis e Permissões

- Corrigida falha de renderização da rota Governança > Perfis e Permissões causada por helper visual inexistente.
- Mantido o rework visual da matriz por categorias e switches ON/OFF.
- Adicionada proteção defensiva para a rota não travar caso ocorra erro visual futuro.
- Preservados backend, API, schema, login, autenticação, permissões reais e arquivos proibidos.

## 1.0.0.20 - Rework Perfis e Permissões

- Reorganizada a matriz de permissões em cards por categoria/módulo, reduzindo leitura crua e excesso de espaço em branco.
- Adicionados cards-resumo por perfil com contagem real de permissões liberadas para Operador, Gestor/PCM e Admin.
- Substituídos checkboxes crus por switches visuais ON/OFF preservando os inputs reais e o salvamento atual.
- Permissões fixas passam a aparecer com badge visual "Fixo", sem alteração de regra de permissão.
- Campo "Motivo da alteração" recebeu layout mais compacto e alinhado ao painel.
- Botão "Salvar permissões" reposicionado em barra de ação clara ao final da matriz.
- Melhorada a densidade visual desktop/mobile sem alterar Módulos, Workflow, backend, API, schema ou permissões reais.
